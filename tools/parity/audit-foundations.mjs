...
const REGISTRY = [
  ...
  { key: 'gap', label: 'Spacing semântico — gap', status: 'check',
    css: () => cssLeaves(/--gap-([a-z0-9]+)\s*:/g),
    figma: () => strip(fig.semanticSpacingVars, 'gap/') },
  { key: 'padding', label: 'Spacing semântico — padding', status: 'check',
    css: () => cssLeaves(/--padding-([a-z0-9]+)\s*:/g),
    figma: () => strip(fig.semanticSpacingVars, 'padding/') },
